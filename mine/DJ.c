...
Motor_Param_t motor[MOTOR_TOTAL_NUM] = {0};  // 电机运行参数

static PID_HandleTypeDef pid[MOTOR_TOTAL_NUM] = {0};// PID参数（静态全局）
static Motor_State_e motor_state = MOTOR_STATE_IDLE;// 电机状态
static uint16_t it_cnt = 0U;                        // 中断计数
...
{
    // 1. 拷贝默认配置到运行参数，初始化变量
    for (uint8_t i = 0; i < MOTOR_TOTAL_NUM; i++)motor[i] = motor_default_config[i];
