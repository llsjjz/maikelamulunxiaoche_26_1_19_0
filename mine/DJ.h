...
#define MOTOR_TIM_IT_HANDLE     &htim7              // 转速计算定时器句柄（100ms中断，0.1s计算一次转速）
...
// 电机状态枚举（替代原DJ_State魔法数字）
typedef enum {
    MOTOR_STATE_IDLE = 0,       // 0：等待指令
    MOTOR_STATE_EXEC = 1,       // 1：执行命令
    MOTOR_STATE_PID = 2         // 2：PID调速中
} Motor_State_e;

// GPIO引脚结构体（封装端口+引脚，语义化）
typedef struct {
...
void DJ_base_run(uint8_t motor_num, int32_t duty);//单个舵机角度控制函数（核心控制逻辑）
